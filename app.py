...
if not st.session_state.setup_complete:
    ...
    with col2:
        st.session_state["position"] = st.selectbox(
            "Choose a position",
            ("Data Scientist", "Data Engineer", "ML Engineer", "BI Analyst", "Finalial Analyst")
        )
...
if st.session_state.setup_complete and not st.session_state.feedback_shown and not st.session_state.chat_complete:
    ...
    if not st.session_state.messages:
        st.session_state.messages = [{"role": "system", "content": f"""
                                    You are a HR executive that interviews an interviewee called {st.session_state["name"]} 
                                    with experience {st.session_state["experience"]} and skills {st.session_state["skills"]}. 
                                    You should interview them for position {st.session_state["level"]} {st.session_state["position"]} 
    ...
    feedback_completion = feedback_client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": """You are a helpful tool that provides feedback on interviewee performance. 
             ...
             """},
             {"role": "user", "content": f"""
              This is the interview you need to evaluate. Keep in mind that you are only a tool and should not engage in conversation: {conversation_history}
              """}        
        ]
    )
